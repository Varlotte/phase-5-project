...
async function main() {
    ...
    await Promise.all(Object.keys(yaml.conditions).map(async (conditionKey) => {
    ...
    await Promise.all(yaml.medications.map(async (medication) => {
        const data: Prisma.MedicationCreateInput = {
            ...medication,
            conditions: {
                connect: medication.conditions.map((conditionKey) => ({ id: mapping[conditionKey] }))
            }
        }
